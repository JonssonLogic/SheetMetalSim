...
| 09-17 | `OutputToLSDyna.py` | Step 7 no longer just reports a blankholder with no clamp force or a punch with no motion - it **asks whether to export anyway** ("Export anyway" / "Cancel export"), the user's choice from three options put to them. Cancelling writes nothing and closes the dialog; step 7 is one click away again once step 5 or 6 has been run. `_check_parts_are_driven()` now returns the problem lines as well as printing them, so the console keeps the detail and the dialog stays readable. If the question cannot be put - no GUI, a refused window - the export **proceeds** rather than being blocked by a dialog nobody can answer, and says so. Eight cases checked offline, including that a clean model is never asked. **Verified in ANSA by the user, 2026-09-17**, both buttons |
| 09-17 | All five dialog scripts | **Every refusal now shows a dialog as well as printing.** Raised by the user after testing: a console-only message is invisible to students, so a refused OK reads as the button doing nothing. `_message()` is duplicated per script like `_typed_number`, wraps `BCMessageWindowCreate`/`Execute`, and swallows its own failures so a GUI problem cannot turn a refusal into a traceback. 16 refusal paths covered - step 2 (2), step 5 (3), step 6 (3), springback export (5), step 7 (3) - each asserted offline to return False **and** raise a dialog. Step 6's "clamp force already exists" also changed from closing the dialog to leaving it open. Step 2's live readout stays console-free: it is feedback while typing, not a refusal. **Verified in ANSA by the user, 2026-09-17** |
| 09-17 | `FixGeoAndMesh.py`, `CreatePrescribedMotion.py`, `CreateClampForce.py` | Every dialog number is read through `_typed_number`: `guitk.constants.blank` is checked for instead of being compared with 0, and a decimal comma is accepted. Step 2's blank size is read on **every keystroke** to refresh the readout, and `None` cannot be compared with the size limits - the readout now says "not a number" instead of raising mid-typing, and OK tells that apart from "out of range". Two further faults in step 5, both found by writing the tests: a stroke of exactly 1 ms made `sf = distance / area` a **division by zero**, and a zero distance was accepted as a punch that never moves - both are now refused with the dialog left open. Step 6 refuses a zero force but **deliberately allows a negative one**, because the sign is what sets which way along the chosen DOF the blankholder pushes. 18 cases checked offline, including that all five copies of `_typed_number` agree |
| 09-17 | `SetPropertyName.py` | Blank thickness is read as a **number** and written to `T1` period-formatted, instead of as the field's raw text. Both `1.5` and `1,5` are accepted - the class runs on Swedish Windows - and `BCLineEditGetDouble`'s `guitk.constants.blank` sentinel is checked for rather than compared with 0. A thickness that is not a number, or is zero or less, is refused with the dialog left open. Previously a comma reached `T1` verbatim, where `CreateContacts._master_thickness` does `float(T1)`: that raises, the blank was reported as having no thickness at all, and step 4 stopped two steps after the value was typed |
| 09-17 | `OutputSpringbackToLSDyna.py` | Export dialog asks for the springback **end time**, default **0.5 s**, and writes it into `implicit-main.k`'s `*CONTROL_TERMINATION` `ENDTIM`. Addressed by field name and right-justified in its 10-character column, so the rest of the card holds its columns whatever number of digits the student types; the deck is not written at all if the field cannot be found. Verified offline against the real deck for eight values from `0.001` to `1.2346E+12`. Previously the deck was copied with `ENDTIM` left at the template's `0.50`. The field takes **either decimal separator**: `BCLineEditGetDouble` returns `guitk.constants.blank` - a sentinel, not a number - when it cannot parse what was typed, so the raw text is re-read with a comma treated as a decimal point. Nothing locale-dependent reaches the deck, since `%`-formatting always writes a period. Ten input cases checked offline |
